Программа определяет, содержит ли введенное пользователем число, цифру 7.
```python
num = int(input()) # ввод числа присваиваем переменную
number = num # присваиваем переменной введенное число, для того чтобы зафиксировать вывод
flag = False # устанавливаем флаг
while num != 0: # пока число не равно 0
    last_digit = num % 10 # присваиваем переменной остаток от деления
    if last_digit == 7: # если остаток от деления равен 7
        flag = True
...
```
Программа, определяющая, что число является простым:
```python
num = int(input())
flag = True
